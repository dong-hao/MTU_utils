...
def read_tsn(fpath, fname):
    ...
    with open(filepath, 'rb') as TSfid:
        ...
        for iblock in range(Nblock):
            ...
            if Nch == 3:
                ts[0, start_idx:end_idx] = getsign24(data[2, :] * p16 + data[1, :] * p8 + data[0, :])  # Ex1
                ts[1, start_idx:end_idx] = getsign24(data[5, :] * p16 + data[4, :] * p8 + data[3, :])  # Ex2
                ts[2, start_idx:end_idx] = getsign24(data[8, :] * p16 + data[7, :] * p8 + data[6, :])  # Ex3
            elif Nch == 4:
                ts[0, start_idx:end_idx] = getsign24(data[2, :] * p16 + data[1, :] * p8 + data[0, :])  # Ex1
                ts[1, start_idx:end_idx] = getsign24(data[5, :] * p16 + data[4, :] * p8 + data[3, :])  # Ex2
                ts[2, start_idx:end_idx] = getsign24(data[8, :] * p16 + data[7, :] * p8 + data[6, :])  # Ex3
                ts[3, start_idx:end_idx] = getsign24(data[14, :] * p16 + data[13, :] * p8 + data[12, :])  # Hy
            elif Nch == 5:
                ts[0, start_idx:end_idx] = getsign24(data[2, :] * p16 + data[1, :] * p8 + data[0, :])  # Ex
                ts[1, start_idx:end_idx] = getsign24(data[5, :] * p16 + data[4, :] * p8 + data[3, :])  # Ey
                ts[2, start_idx:end_idx] = getsign24(data[8, :] * p16 + data[7, :] * p8 + data[6, :])  # Hx
                ts[3, start_idx:end_idx] = getsign24(data[11, :] * p16 + data[10, :] * p8 + data[9, :])  # Hy
                ts[4, start_idx:end_idx] = getsign24(data[14, :] * p16 + data[13, :] * p8 + data[12, :])  # Hz
            elif Nch == 6:
                ts[0, start_idx:end_idx] = getsign24(data[2, :] * p16 + data[1, :] * p8 + data[0, :])  # Ex
                ts[1, start_idx:end_idx] = getsign24(data[5, :] * p16 + data[4, :] * p8 + data[3, :])  # Ey
                ts[2, start_idx:end_idx] = getsign24(data[8, :] * p16 + data[7, :] * p8 + data[6, :])  # Ez
                ts[3, start_idx:end_idx] = getsign24(data[11, :] * p16 + data[10, :] * p8 + data[9, :])  # Hx
                ts[4, start_idx:end_idx] = getsign24(data[14, :] * p16 + data[13, :] * p8 + data[12, :])  # Hy
                ts[5, start_idx:end_idx] = getsign24(data[17, :] * p16 + data[16, :] * p8 + data[15, :])  # Hz
